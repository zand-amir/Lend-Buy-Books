...
r = requests.post('http://localhost:8000/api/Books/Buy/',json={"OfferID":1})
print('result :')
print(r.json())
...
print('Buying the book ' + '...')
r = requests.post('http://localhost:8000/api/Books/Buy/',json={"OfferID":1},headers=hed)
print('result :')
print(r.json())
print('status code : '+str(r.status_code)+'\n')

print('3rd Test : logged in as a valid user but the request has an invalid offer id')
print('expected status code : 400')
r = requests.post('http://localhost:8000/api/User/token/',json=random.choice(users))
access = r.json()['access']
hed = {'Authorization':'Bearer '+access}
print('Buying the book ' +  '...')
r = requests.post('http://localhost:8000/api/Books/Buy/',json={"OfferID":243},headers=hed)
print('result :')
print(r.json())
print('status code : '+str(r.status_code)+'\n')

print('4th Test : proper request(selected user may or may not have enough credit)')
